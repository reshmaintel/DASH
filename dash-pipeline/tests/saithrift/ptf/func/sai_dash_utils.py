...
class VnetAPI(VnetObjects):
    ...
    def inbound_routing_decap_validate_create(self, eni_id, vni, sip, sip_mask, src_vnet_id):
        ...
        inbound_routing_entry = sai_thrift_inbound_routing_entry_t(
            ...
            sip_mask=sai_ipaddress(sip_mask),
            priority=0)
...
class VnetTrafficMixin:
    ...
    def verify_bidirectional_connection(self,
                                        ...
                                        connection: str,
                                        fake_mac=True, tx_equal_to_rx=True,
    ...
    def create_vxlan_tcp_session_packets(self,
        ...
        client_inner_pkt = simple_tcp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                             eth_src=client.client.mac,
                                             ip_dst=server.client.ip,
                                             ip_src=client.client.ip,
                                             tcp_sport=client_tcp_port,
                                             tcp_dport=http_port,
                                             tcp_flags=self.SYN)
        self.update_tcp_pkt(client_inner_pkt, seq=1, ack=0)

        client_tcp_syn_vxlan_pkt = simple_vxlan_packet(eth_dst=client.peer.mac,
                                                       eth_src=client.mac,
                                                       ip_dst=client.peer.ip,  # VIP
                                                       ip_src=client.ip,
                                                       with_udp_chksum=False,
                                                       vxlan_vni=client.client.vni,
                                                       inner_frame=client_inner_pkt)

        exp_client_inner_pkt = simple_tcp_packet(eth_dst=server.client.mac,
                                                 eth_src=client.client.mac,
                                                 ip_dst=server.client.ip,
                                                 ip_src=client.client.ip,
                                                 tcp_sport=client_tcp_port,
        ...
        exp_client_tcp_syn_vxlan_pkt = simple_vxlan_packet(eth_dst=server.mac,
                                                           eth_src=server.peer.mac,
                                                           ip_dst=server.ip,
                                                           ip_src=server.peer.ip,  # VIP
                                                           with_udp_chksum=False,
        ...
        server_inner_pkt = simple_tcp_packet(eth_dst=client.client.mac,
                                             eth_src=server.client.mac,
                                             ip_dst=client.client.ip,
                                             ip_src=server.client.ip,
                                             tcp_sport=http_port,
                                             tcp_dport=client_tcp_port,
                                             tcp_flags=self.SYN_ACK)
        self.update_tcp_pkt(server_inner_pkt, seq=10, ack=2)

        server_tcp_synack_vxlan_pkt = simple_vxlan_packet(eth_dst=server.peer.mac,
                                                          eth_src=server.mac,
                                                          ip_dst=server.peer.ip,  # VIP
                                                          ip_src=server.ip,
                                                          with_udp_chksum=False,
                                                          vxlan_vni=server.client.vni,
                                                          inner_frame=server_inner_pkt)

        exp_server_tcp_synack_vxlan_pkt = simple_vxlan_packet(eth_dst=client.mac,
                                                              eth_src=client.peer.mac,
                                                              ip_dst=client.ip,
                                                              ip_src=client.peer.ip,  # VIP
                                                              with_udp_chksum=False,
                                                              vxlan_vni=client.client.vni,
                                                              inner_frame=server_inner_pkt)
    ...
    def create_vxlan_tcp_session_route_direct_packets(self,
        ...
        exp_client_tcp_syn_pkt = simple_tcp_packet(eth_dst=server.mac,
                                                   eth_src=server.peer.mac,
                                                   ip_dst=server.client.ip,
                                                   ip_src=client.peer.ip,
                                                   tcp_sport=client_tcp_port,
                                                   tcp_dport=http_port,
                                                   tcp_flags=self.SYN)
        ...
        # Server (host2) packets
        # create tcp SYN ACK pkt encapsulated in VxLAN packet
        server_tcp_synack_pkt = simple_tcp_packet(eth_dst=client.peer.mac,
                                                  eth_src=server.client.mac,
                                                  ip_dst=client.peer.ip,
                                                  ip_src=server.client.ip,
                                                  tcp_sport=http_port,
                                                  tcp_dport=client_tcp_port,
                                                  tcp_flags=self.SYN_ACK)
        ...
        exp_server_inner_pkt = simple_tcp_packet(eth_dst=client.client.mac,
                                                 eth_src=server.client.mac,
                                                 ip_dst=client.client.ip,
                                                 ip_src=server.client.ip,
                                                 tcp_sport=http_port,
    ...
    @staticmethod
    def create_vxlan_udp_session_packets(client: DutNeighborNetworkParameters,
                                         server: DutNeighborNetworkParameters,
                                         fake_mac: bool):
        """
        ...
        # Client (host1) packets
        client_inner_pkt = simple_udp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                             eth_src=client.client.mac,
                                             ip_dst=server.client.ip,
                                             ip_src=client.client.ip,
        ...
        client_vxlan_pkt = simple_vxlan_packet(eth_dst=client.peer.mac,
                                               eth_src=client.mac,
                                               ip_dst=client.peer.ip,  # VIP
                                               ip_src=client.ip,
                                               with_udp_chksum=False,
        ...
        exp_client_inner_pkt = simple_udp_packet(eth_dst=server.client.mac,
                                                 eth_src=client.client.mac,
                                                 ip_dst=server.client.ip,
                                                 ip_src=client.client.ip,
                                                 udp_sport=client_udp_port,
        ...
        exp_client_vxlan_pkt = simple_vxlan_packet(eth_dst=server.mac,
                                                   eth_src=server.peer.mac,
                                                   ip_dst=server.ip,
                                                   ip_src=server.peer.ip,  # VIP
                                                   with_udp_chksum=False,
        ...
        # Server (host2) packets
        server_inner_pkt = simple_udp_packet(eth_dst=client.client.mac,
                                             eth_src=server.client.mac,
                                             ip_dst=client.client.ip,
                                             ip_src=server.client.ip,
                                             udp_sport=http_port,
        ...
        server_vxlan_pkt = simple_vxlan_packet(eth_dst=server.peer.mac,
                                               eth_src=server.mac,
                                               ip_dst=server.peer.ip,  # VIP
                                               ip_src=server.ip,
                                               with_udp_chksum=False,
        ...
        exp_server_vxlan_pkt = simple_vxlan_packet(eth_dst=client.mac,
                                                   eth_src=client.peer.mac,
                                                   ip_dst=client.ip,
                                                   ip_src=client.peer.ip,  # VIP
                                                   with_udp_chksum=False,
    ...
    @staticmethod
    def create_vxlan_icmp_session_packets(client: DutNeighborNetworkParameters,
                                          server: DutNeighborNetworkParameters,
                                          fake_mac: bool):
        """
        ...
        # Client (host1) Echo request
        client_inner_pkt = simple_icmp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                              eth_src=client.client.mac,
                                              ip_dst=server.client.ip,
                                              ip_src=client.client.ip,
                                              icmp_type=8)
        ...
        exp_client_inner_pkt = simple_icmp_packet(eth_dst=server.client.mac,
                                                  eth_src=client.client.mac,
                                                  ip_dst=server.client.ip,
                                                  ip_src=client.client.ip,
                                                  icmp_type=8)
        ...
        # Server (host2) Echo reply
        server_inner_pkt = simple_icmp_packet(eth_dst=client.client.mac,
                                              eth_src=server.client.mac,
                                              ip_dst=client.client.ip,
                                              ip_src=server.client.ip,
                                              icmp_type=0)
    ...
    def create_vxlan_oneway_pkts(self,
        ...
        if conn_type == 'tcp':
            inner_pkt = simple_tcp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                          eth_src=client.client.mac,
                                          ip_dst=server.client.ip,
                                          ip_src=client.client.ip,
            ...
            exp_inner_pkt = simple_tcp_packet(eth_dst=server.client.mac,
                                              eth_src=client.client.mac,
                                              ip_dst=server.client.ip,
                                              ip_src=client.client.ip,
                                              tcp_sport=client_port,
            ...
            inner_pkt = simple_udp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                          eth_src=client.client.mac,
                                          ip_dst=server.client.ip,
                                          ip_src=client.client.ip,
            ...
            exp_inner_pkt = simple_udp_packet(eth_dst=server.client.mac,
                                              eth_src=client.client.mac,
                                              ip_dst=server.client.ip,
                                              ip_src=client.client.ip,
                                              udp_sport=client_port,
            ...
            inner_pkt = simple_icmp_packet(eth_dst=fake_ca_dst_mac if fake_mac else server.client.mac,
                                           eth_src=client.client.mac,
                                           ip_dst=server.client.ip,
                                           ip_src=client.client.ip,
                                           icmp_type=8)

            exp_inner_pkt = simple_icmp_packet(eth_dst=server.client.mac,
                                               eth_src=client.client.mac,
                                               ip_dst=server.client.ip,
                                               ip_src=client.client.ip,
                                               icmp_type=8)
        ...
        vxlan_pkt = simple_vxlan_packet(eth_dst=client.peer.mac,
                                        eth_src=client.mac,
                                        ip_dst=client.peer.ip,  # VIP
                                        ip_src=client.ip,
                                        with_udp_chksum=False,
        ...
        exp_vxlan_pkt = simple_vxlan_packet(eth_dst=server.mac,
                                            eth_src=server.peer.mac,
                                            ip_dst=server.ip,
                                            ip_src=server.peer.ip,  # VIP
                                            with_udp_chksum=False,
